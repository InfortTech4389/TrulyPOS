...
function initializeMegaMenu() {
    // New mega menu functionality with categories and subcategories
    ...
    if (categoryItems.length > 0) {
        categoryItems[0].classList.add('active');
        const firstIndustry = categoryItems[0].dataset.industry;
    ...
    if (dropdownToggle) {
        dropdownToggle.addEventListener('click', function(e) {
            if (window.innerWidth <= 768) {
                e.preventDefault();
                const isVisible = megaMenu.classList.contains('show');
    ...
    window.addEventListener('resize', function() {
        if (window.innerWidth > 768) {
            ...
            businessCategories.forEach(c => c.classList.remove('active'));
    ...
    // Close dropdown when clicking outside
    document.addEventListener('click', function(e) {
        if (megaMenu && !e.target.closest('.mega-dropdown')) {
            megaMenu.classList.remove('show');
        }
    });
}
